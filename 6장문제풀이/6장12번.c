...
void count_node(ListNode *head, int data){
    int count = 0; //노드 개수를 세는 변수
    for(ListNode *p=head; p!=NULL; p=p->link){ //노드를 처음부터 접근하기 위한 반복문
        if(p->data==data){ //원하는 데이터와 노드데이터가 같으면
            count++; //count 증가
